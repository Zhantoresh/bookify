  ...
  return (
    <div>
      <div style={styles.header}>
        <h1 style={styles.h1}>Панель администратора</h1>
        <Link to="/admin/users" style={styles.usersBtn}>Управление пользователями →</Link>
      </div>

      {/* Пользователи */}
      <h2 style={styles.h2}>Пользователи</h2>
      <div style={styles.grid}>
        <StatCard label="Всего"        value={s.total_users}    color="#6366f1" />
        <StatCard label="Клиентов"     value={s.total_clients}  color="#0ea5e9" />
        <StatCard label="Провайдеров"  value={s.total_providers} color="#10b981" />
        <StatCard label="Админов"      value={s.total_admins}   color="#f59e0b" />
      </div>

      {/* Услуги */}
      <h2 style={styles.h2}>Услуги</h2>
      <div style={styles.grid}>
        <StatCard label="Всего услуг"   value={s.total_services}  color="#8b5cf6" />
        <StatCard label="Активных"      value={s.active_services} color="#10b981" />
      </div>

      {/* Записи */}
      <h2 style={styles.h2}>Записи</h2>
      <div style={styles.grid}>
        <StatCard label="Всего"        value={s.total_appointments}     color="#64748b" />
        <StatCard label="Ожидают"      value={s.pending_appointments}   color="#f59e0b" />
        <StatCard label="Подтверждены" value={s.confirmed_appointments} color="#10b981" />
        <StatCard label="Отменены"     value={s.cancelled_appointments} color="#ef4444" />
        <StatCard label="Завершены"    value={s.completed_appointments} color="#6366f1" />
      </div>

      {/* Последние пользователи */}
      {data.recent_users?.length > 0 && (
        <>
          <h2 style={styles.h2}>Последние пользователи</h2>
          <div style={styles.table}>
            <div style={styles.tableHeader}>
              <span>Имя</span><span>Email</span><span>Роль</span>
            </div>
